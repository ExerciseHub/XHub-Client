...
async function getUserInfo() {
    try {
        const response = await axios.get(`https://www.exercisehub.xyz/player/check/email/`, {
            headers: {
...
async function getMeetingDetail(meetingId) {
    try {
        const response = await axios.get(`https://www.exercisehub.xyz/quickmatch/${meetingId}/detail/`);
        return response.data;
    } catch (error) {
...
async function checkMembership(meetingId) {
    try {
        const response = await axios.get(`https://www.exercisehub.xyz/quickmatch/is_member/${meetingId}/`, {
            headers: {
                'Authorization': `Bearer ${localStorage.getItem('accessToken')}`
            }
        });
...
async function deleteMeeting() {
    ...
    // 삭제 요청
    try {
        const response = await axios.post(`https://www.exercisehub.xyz/quickmatch/${value}/delete/`, {}, {
            headers : {
                'Authorization': `Bearer ${localStorage.getItem('accessToken')}`
            }
        });
...
async function statusChange() {
    ...
    try {
        const response = await axios.post(`https://www.exercisehub.xyz/quickmatch/${value}/status/`, {status: newStatus}, {
            headers: {
                'Authorization': `Bearer ${localStorage.getItem('accessToken')}`
            }
        });
        ...
        if (response.status === 200) {
            ...
            const detailResponse = await axios.get(`https://www.exercisehub.xyz/quickmatch/${value}/detail/`);
...
async function attendMeeting() {
    ...
    try {
        const response = await axios.post(`https://www.exercisehub.xyz/quickmatch/join/${value}/`, {}, {
            headers: {
                'Authorization': `Bearer ${localStorage.getItem('accessToken')}`
            }
        });
...
async function leaveMeeting() {
    ...
    try {
        const response = await axios.post(`https://www.exercisehub.xyz/quickmatch/leave/${value}/`, {}, {
            headers: {
                'Authorization': `Bearer ${localStorage.getItem('accessToken')}`
            }
        });
        ...
        if (response.status === 200) {
            ...
            return axios.get(`https://www.exercisehub.xyz/quickmatch/${value}/detail/`);
        } else if(response.status === 401 ){
            // UnAuthorization
            if (localStorage.getItem('userEmail')){
                tokenRefresh();
...
async function evaluateMember(memberId, meetingId) {
    try {
        const response = await axios.post(`https://www.exercisehub.xyz/quickmatch/evaluate_member/${memberId}/${meetingId}/`, {}, {
            headers: {
                'Authorization': `Bearer ${localStorage.getItem('accessToken')}`
            }
        });
